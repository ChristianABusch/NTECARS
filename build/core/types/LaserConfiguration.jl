...
lasers = LaserConfiguration(
    wavelength_1    = 532e-9,
    laser_1_profile = gaussian_profile(1.42*0.64/2.35),
    wavelength_2    = 532e-9,
    laser_2_profile = gaussian_profile(1.42*0.64/2.35),
...
struct LaserConfiguration
    ...
    function LaserConfiguration(;
        ...
        stokes_spectrum::Union{FlatProfile, Spectrum} = FlatProfile()
    )
        if stokes_spectrum isa FlatProfile
            stokes_spectrum = Spectrum([stokes_range...], [1.0, 1.0], :wavelength)
        end
        
        ν_1 = wavelength_to_wavenumber(wavelength_1)
        ν_2 = wavelength_to_wavenumber(wavelength_2)
        ν_S = wavenumbers(stokes_spectrum)
        ν_aS = ν_1 .+ ν_2 .- ν_S
        new(ν_1, ν_2, profile_1, profile_2, stokes_spectrum, extrema(ν_aS))
    end
end
